...
1. Follow [the instructions](https://syllabus.migracode.org/git) to fork & clone the GitHub repo
...
**Instructions:** Create a `<SearchResults />` component with an empty table where later in the exercise we will put hotel bookings in a `<table>` element. The table header should have fields for `id`, `title`, `first name`, `surname`, `email`, `room id`, `check in date` and `check out date`. If you like, you can make up data for a few bookings in the `<SearchResults />` component, so that these show up in the table. Then show `<SearchResults />` component within the `<Bookings />` component that is provided. Be sure to split out your components into small well-named components, similar to the method used in exercise 1.
...
**Hint:** Use a new state variable for each row to record if the row is selected or not, and use this value to set a class to the `className` pr op of the row, the className value can contain an usestate and change its value depending on it
